...
open class _CountDownInlineRow: _DateInlineFieldRow {
    ...
    public required init(tag: String?) {
        ...
        displayValueFor =  {
            ...
            let hour = (Calendar.current as NSCalendar).component(.hour, from: date as Date)
            let min = (Calendar.current as NSCalendar).component(.minute, from: date as Date)
            if hour == 1{
    ...
    open func setupInlineRow(_ inlineRow: CountDownPickerRow) {
        configureInlineRow(inlineRow)
    }
}
